 ...
 * 
 * IMPORTANT: This engine uses the swisseph native module which can ONLY run in Node.js
 * For browser/Next.js usage, use the API route or separate Node.js script
 * 
 * To test: node scripts/test-swisseph.js
 * To use in production: Run calculations via API or separate microservice
 */

// NOTE: swisseph is a native module and can only be used in Node.js environment
// For browser usage, we provide fallback calculations
let swisseph: any = null;

try {
  swisseph = require('swisseph');
} catch (error) {
  console.log('⚠️  swisseph module not available in browser environment');
  console.log('ℹ️  Using fallback calculations for demo purposes');
}

// Planetary constants (matching swisseph indices)
const PLANETS = swisseph ? {
  SUN: swisseph.SE_SUN,
  ...
  // Fallback constants for browser environment
  SUN: 0,
  MOON: 1,
  MERCURY: 2,
  VENUS: 3,
...
// Ayanamsha modes
const AYANAMSHA_MODES = swisseph ? {
...
export class SwissEphemerisEngine {
  ...
  constructor(ephemerisPath: string = './ephe', useKPSystem: boolean = true) {
    ...
    this.isNodeEnvironment = typeof process !== 'undefined' && process.versions && process.versions.node !== undefined;
  }
  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (!this.isNodeEnvironment) {
        ...
        console.log('ℹ️  Using fallback calculations (not real ephemeris)');
        this.isInitialized = true;
        return;
      }
      
      if (!swisseph) {
        throw new Error('swisseph module not available. Please run: npm install swisseph');
      }
      
      // Set ephemeris file path for swisseph
      swisseph.swe_set_ephe_path(this.ephemerisPath);
      
      // Set ayanamsha to LAHIRI for Vedic astrology
      ...
      console.log('⚠️  Continuing with fallback calculations');
  ...
  async calculateEphemerisForBTR(
    ...
    try {
      ...
      // Use real swisseph if available, otherwise use fallbacks
      if (this.isNodeEnvironment && swisseph) {
        return await this.calculateWithSwisseph(date, latitude, longitude, houseSystem);
      } else {
        console.log('⚠️  Using fallback calculations for browser environment');
        return await this.calculateWithFallbacks(date, latitude, longitude, houseSystem);
      }
  ...
  /**
   * Calculate using real swisseph (Node.js only)
   */
  private async calculateWithSwisseph(date: Date, latitude: number, longitude: number, houseSystem: string): Promise<EphemerisCalculation> {
    if (!swisseph) {
      throw new Error('swisseph not available');
    }
  ...
  /**
   * Fallback calculations for browser environment
   */
  private async calculateWithFallbacks(date: Date, latitude: number, longitude: number, houseSystem: string): Promise<EphemerisCalculation> {
    const julianDay = this.calculateJulianDay(date);
    
    // Use fallback planetary positions (approximate)
    const planets = this.calculateFallbackPlanets(date, latitude, longitude);
    
    // Use fallback house cusps
    const houseCusps = this.calculateHouseCuspsFallback(date, latitude, longitude, houseSystem);
  ...
  /**
   * Fallback planetary positions (approximate for demo)
   */
  private calculateFallbackPlanets(date: Date, latitude: number, longitude: number): any {
    const dayOfYear = this.getDayOfYear(date);
    const year = date.getUTCFullYear();
    
    // Approximate positions (these are NOT accurate, just for demo)
    const basePositions: Record<string, number> = {
      sun: (dayOfYear / 365.25 * 360) % 360,
      moon: (dayOfYear / 27.32 * 360) % 360, // Lunar month
      mercury: (dayOfYear / 88 * 360 + 30) % 360, // Mercury year
      venus: (dayOfYear / 225 * 360 + 60) % 360, // Venus year
      mars: (dayOfYear / 687 * 360 + 90) % 360, // Mars year
      jupiter: ((year - 1900) * 30 + dayOfYear / 365.25 * 30) % 360, // Approximate
      saturn: ((year - 1900) * 12 + dayOfYear / 365.25 * 12) % 360  // Approximate
    };
    
    const planets: any = {};
    
    Object.keys(basePositions).forEach(planetName => {
      const longitude = basePositions[planetName];
      planets[planetName] = this.createPlanetaryPosition(
        longitude,
        0,
        1.0,
        false,
    ...
    if (planets.rahu) {
      ...
      planets.ketu = this.createPlanetaryPosition(
        ketuLongitude,
        0,
        -0.053,
        true,
  ...
  private async calculatePlanetaryPositions(date: Date, latitude: number, longitude: number): Promise<any> {
    if (!swisseph) {
      return this.calculateFallbackPlanets(date, latitude, longitude);
    }
    ...
    const planetsToCalculate = [
      ...
      { name: 'rahu', id: PLANETS.TRUE_NODE }, // True Node for accurate Vedic astrology
    ];
  ...
  private async calculateHouseCusps(date: Date, latitude: number, longitude: number, houseSystem: string): Promise<HouseCusps> {
    ...
    try {
      // Calculate houses using swisseph
  ...
  private calculateNavamsa(signIndex: number, degreeInSign: number): number {
    ...
    const navamsaSize = 3.3333333333333335;
    ...
    // Fixed signs (Taurus, Leo, Scorpio, Aquarius): start from 9th from sign
      ...
      navamsaSign = (signIndex + 8 + navamsaNumber) % 12;
    }
    // Dual signs (Gemini, Virgo, Sagittarius, Pisces): start from 5th from sign
    else {
      navamsaSign = (signIndex + 4 + navamsaNumber) % 12;
    }
  ...
  private calculateDasamsa(signIndex: number, degreeInSign: number): number {
    ...
    // Even signs: start from 9th from sign
      ...
      dasamsaSign = (signIndex + 8 + dasamsaNumber) % 12;
    }
  ...
  private calculateSaptamsa(signIndex: number, degreeInSign: number): number {
    ...
    const saptamsaSize = 4.285714285714286;
    ...
    // Even signs: start from 7th from sign
      ...
      saptamsaSign = (signIndex + 6 + saptamsaNumber) % 12;
    }
  ...
  private getTrimsamsaSign(signIndex: number, degreeInSign: number): number {
    ...
    if (isOddSign) {
      if (degree >= 0 && degree < 5) return 4; // Mars
      if (degree >= 5 && degree < 10) return 2; // Venus
      if (degree >= 10 && degree < 18) return 6; // Mercury
      if (degree >= 18 && degree < 25) return 5; // Jupiter
      if (degree >= 25 && degree < 30) return 7; // Saturn
    } else {
      if (degree >= 0 && degree < 5) return 2; // Venus
      if (degree >= 5 && degree < 12) return 4; // Mars
      if (degree >= 12 && degree < 20) return 6; // Mercury
      if (degree >= 20 && degree < 25) return 7; // Saturn
      if (degree >= 25 && degree < 30) return 5; // Jupiter
    }
  ...
  /**
   * Calculate birth Pratyantardasha
   */
  ...
  private getZodiacSign(degree: number): string {
    return ZODIAC_SIGNS[Math.floor(degree / 30)] || 'Unknown';
  }
  ...
  private calculateEnhancedSthanaBala(planet: string, planetData: PlanetaryPosition, chartData: EphemerisCalculation): number {
    ...
    const exaltationPoints: Record<string, number> = {
      'Sun': 10, 'Moon': 33, 'Mars': 298, 'Mercury': 163,
      'Jupiter': 95, 'Venus': 337, 'Saturn': 200, 'Rahu': 180, 'Ketu': 0
    };
    ...
    for (const division of divisionsToCheck) {
      ...
      const dignityScore: Record<string, number> = {
        'exalted': 60, 'own_sign': 45, 'friendly': 30,
        'neutral': 15, 'enemy': 8, 'debilitated': 0
      };
    ...
    // 3. Ojhajugmariamsa Bala (Odd-even sign strength)
  ...
  private calculateEnhancedDigBala(planet: string, planetData: PlanetaryPosition): number {
    const directionalStrength: Record<string, number> = {
      'Sun': 90, 'Moon': 0, 'Mars': 90, 'Mercury': 10,
      'Jupiter': 10, 'Venus': 180, 'Saturn': 270, 'Rahu': 270, 'Ketu': 90
    };
    
    const preferredDirection = directionalStrength[planet] || 0;
    let angularDistance = Math.abs(planetData.longitude - preferredDirection);
  ...
  private calculateEnhancedKaalaBala(planet: string, planetData: PlanetaryPosition, chartData: EphemerisCalculation): number {
    ...
    if (planet === 'Mercury') natonnataBala = 45;
    ...
    if (planet === 'Mercury') pakshaBala = 45;
    
    strength += pakshaBala * 0.3;
    
    // Tribhaga Bala
    const isFirstTribhaga = hour >= 0 && hour < 8;
    const isSecondTribhaga = hour >= 8 && hour < 16;
    const isThirdTribhaga = hour >= 16 && hour < 24;
    
    let tribhagaBala = 30;
    if (planet === 'Sun' && isFirstTribhaga) tribhagaBala = 60;
    else if (planet === 'Mars' && isSecondTribhaga) tribhagaBala = 60;
    else if (planet === 'Jupiter' && isThirdTribhaga) tribhagaBala = 60;
  ...
  private calculateEnhancedChestaBala(planet: string, planetData: PlanetaryPosition): number {
    if (['Sun', 'Moon'].includes(planet)) return 0;
    
    const meanSpeeds: Record<string, number> = {
      'Mercury': 0.98, 'Venus': 0.62, 'Mars': 0.52,
    ...
    if (planetData.retrograde) {
      ...
      if (speedRatio > 1.5) chestaBala = 80;
      else if (speedRatio > 1.2) chestaBala = 70;
      else if (speedRatio > 0.8) chestaBala = 60;
      else if (speedRatio > 0.5) chestaBala = 50;
      else chestaBala = 40;
    }
  ...
  private calculateEnhancedNaisargikaBala(planet: string): number {
    const naturalStrength: Record<string, number> = {
      ...
      'Mercury': 25.71, 'Mars': 17.14, 'Saturn': 8.57, 'Rahu': 8.57, 'Ketu': 8.57
    };
  ...
  private calculateEnhancedDrikBala(planet: string, planetData: PlanetaryPosition, chartData: EphemerisCalculation): number {
    ...
    const aspectStrengths: Record<string, number[]> = {
      'Sun': [180], 'Moon': [180], 'Mars': [120, 180, 270],
      'Mercury': [180], 'Jupiter': [60, 180, 300],
      'Venus': [180], 'Saturn': [90, 180, 360],
      'Rahu': [180], 'Ketu': [180]
    };
    ...
    for (const [otherPlanet, otherData] of Object.entries(chartData.planets)) {
      ...
      for (const aspect of aspects) {
        const aspectDiff = this.calculateAspectDifference(planetData.longitude, (otherData as any).longitude, aspect);
        
        if (aspectDiff !== null) {
          const orbStrength = Math.max(0, 1 - (aspectDiff / 15));
          
          if (benefics.includes(otherPlanetName)) {
            beneficInfluence += orbStrength * 15;
          } else if (malefics.includes(otherPlanetName)) {
            maleficInfluence += orbStrength * 12;
          }
        }
      }
    }
  ...
  /**
   * Check if two planets are connected
   */
