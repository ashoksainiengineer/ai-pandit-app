...
case "$SERVICE_KIND" in
  ...
  worker)
    ...
    EXTRA_VARS="DB_POOL_MAX=3,NODE_ENV=production,BTR_TIMEOUT_MS=${BTR_TIMEOUT_MS:-14400000},APP_REGION=${REGION},CLOUD_RUN_REGION=${REGION},JOB_EXECUTION_MODE=external_worker,WORKER_POLL_INTERVAL_MS=${WORKER_POLL_INTERVAL_MS:-2000},WORKER_DRAIN_TIMEOUT_MS=${WORKER_DRAIN_TIMEOUT_MS:-30000},JOB_SYNC_POLL_INTERVAL_MS=${JOB_SYNC_POLL_INTERVAL_MS:-60000},USE_ASYNC_JOB_PIPELINE=${USE_ASYNC_JOB_PIPELINE:-true},USE_NEW_STREAM_PATH=${USE_NEW_STREAM_PATH:-true},AI_BASE_URL=${AI_BASE_URL:-https://api.groq.com/openai/v1},AI_MODEL=${AI_MODEL:-openai/gpt-oss-120b},AI_REASONING_MODE=${AI_REASONING_MODE:-include_reasoning},AI_PARALLEL_CONCURRENCY=${AI_PARALLEL_CONCURRENCY:-3},AI_STAGE2_MAX_TOKENS=${AI_STAGE2_MAX_TOKENS:-8192},AI_STAGE4_MAX_TOKENS=${AI_STAGE4_MAX_TOKENS:-8192},AI_STAGE6_MAX_TOKENS=${AI_STAGE6_MAX_TOKENS:-8192},EPHEMERIS_PROVIDER=${EPHEMERIS_PROVIDER:-skyfield},EPHEMERIS_ALLOW_ALGORITHMIC_FALLBACK=${EPHEMERIS_ALLOW_ALGORITHMIC_FALLBACK:-false},EPHEMERIS_SERVICE_URL=https://ephemeris-service-7tjuxigfoq-as.a.run.app,EPHEMERIS_SERVICE_TIMEOUT_MS=${EPHEMERIS_SERVICE_TIMEOUT_MS:-15000},EPHEMERIS_BATCH_SIZE=${EPHEMERIS_BATCH_SIZE:-250},EPHEMERIS_HOUSE_SYSTEM=${EPHEMERIS_HOUSE_SYSTEM:-whole_sign},REDIS_TLS=${REDIS_TLS:-true},NODE_OPTIONS=--max-old-space-size=1024"
    SECRET_VARS="NEON_DATABASE_URL=neon-database-url:latest,REDIS_URL=redis-url:latest,AI_API_KEY=groq-api-key:latest,ENCRYPTION_SECRET=encryption-secret:latest,CLERK_SECRET_KEY=clerk-secret-key:latest"
    ;;
  ephemeris)
