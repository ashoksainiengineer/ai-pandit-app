...
function calculateTatwaShuddhi(birthJd, sunriseJd, gender = 'male') {
    ...
    if (gender === 'male' && ['Fire', 'Air', 'Ether'].includes(currentTatwa))
        score = 85;
    if (gender === 'female' && ['Earth', 'Water'].includes(currentTatwa))
        score = 85;
    return {
        passed: score >= 50,
        score,
...
function calculateVarnadaLagna(ephemeris) {
    ...
    if (waterSigns.includes(sign))
        return 'Brahmin (Knowledge/Healing)';
    if (fireSigns.includes(sign))
        return 'Kshatriya (Power/Strategy)';
    if (earthSigns.includes(sign))
        return 'Vaishya (Commerce/Business)';
    return 'Shudra (Expert Service/Execution)';
}
//# sourceMappingURL=shuddhi-engine.js.map