...
async function buildCandidateDataPackage(time, offsetMinutes, input, includeFullData = false) {
    ...
    for (const [key, p] of Object.entries(ephemeris.planets)) {
        ...
        richPlanets[key] = {
            ...
            house: (0, vedic_astrology_engine_js_1.calculateHouse)(ephemeris.ascendant.sign, p.sign),
            dignity: (0, vedic_astrology_engine_js_1.getDignity)(planetName, p.sign),
            isRetro: p.retro
        };
    }
    const pkg = {
...
${c.transitData ? `  TRANSITS: ${Object.entries(c.transitData).slice(0, 5).map(([date, t]) => `[${date}]: Sa=${t.saturn}, Ju=${t.jupiter}, Ra=${t.rahu}`).join(' | ')}` : ''}`).join('\n')}
    ...
    while (currentCandidates.length > batchSize) {
        ...
        for (let i = 0; i < batches.length; i++) {
            ...
            for (const candidate of batch) {
                ...
                (0, session_events_js_1.emitCandidateScore)(input.sessionId, candidate.time, score, 2, undefined, getMinifiedEph(candidate) // 👈 FIX: Pass Eph Data
                );
            }
