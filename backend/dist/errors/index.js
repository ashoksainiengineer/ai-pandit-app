...
export class NotFoundError extends AppError {
    constructor(resource, identifier) {
        super(ErrorCodes.RESOURCE_NOT_FOUND, `${resource} not found${identifier ? `: ${identifier}` : ''}`, identifier ? { resource, identifier } : { resource });
    }
}
export class SessionNotFoundError extends AppError {
    constructor(sessionId) {
        super(ErrorCodes.SESSION_NOT_FOUND, `Session not found: ${sessionId}`, { sessionId });
    }
}
export class RateLimitError extends AppError {
    constructor(retryAfter) {
        super(ErrorCodes.RATE_LIMIT_EXCEEDED, 'Rate limit exceeded. Please try again later.', retryAfter ? { retryAfter } : undefined);
    }
}
...
export class AITimeoutError extends AppError {
    constructor(timeoutMs) {
        super(ErrorCodes.AI_TIMEOUT, `AI service timeout after ${timeoutMs}ms`, { timeoutMs });
    }
}
export class QueueFullError extends AppError {
    constructor(queueSize, maxSize) {
        super(ErrorCodes.QUEUE_FULL, 'Processing queue is full. Please try again later.', { queueSize, maxSize, availableSlots: maxSize - queueSize });
    }
}
export class ProcessingError extends AppError {
    constructor(message, sessionId) {
        super(ErrorCodes.PROCESSING_ERROR, message, sessionId ? { sessionId } : undefined);
    }
}
...
export class CancellationError extends AppError {
    constructor(sessionId) {
        super(ErrorCodes.REQUEST_CANCELLED, 'Request was cancelled by user', { sessionId });
    }
}
// ═════════════════════════════════════════════════════════════════════════════
// ERROR TYPE GUARDS
...
export function handleUnknownError(error) {
    ...
    if (error instanceof Error) {
        return new AppError(ErrorCodes.INTERNAL_ERROR, error.message, { originalError: error.name }, false);
    }
    return new AppError(ErrorCodes.INTERNAL_ERROR, 'An unexpected error occurred', { originalError: String(error) }, false);
}
