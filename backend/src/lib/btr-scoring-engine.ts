...
import { calculateD60, calculateD9, calculateD10 } from './advanced-btr-methods.js';
...
const EVENT_SIGNIFICATORS: Record<string, EventSignificators> = {
  marriage: {
...
export class BTRScoringEngine {
  ...
  private ephemeris: EphemerisData;
  private events: LifeEvent[];
  private forensicProfile: any;
  ...
  public calculateScore(): ScoringResult {
    ...
    const weightedTotal = 
      dashaScore.score * this.weights.dashaEventCorrelation +
  ...
  private calculateDashaEventCorrelation(): { 
    score: number; 
    details: EventMatchDetail[];
    dashaDetails: DashaAnalysisDetail[];
  } {
    const details: EventMatchDetail[] = [];
    const dashaDetails: DashaAnalysisDetail[] = [];
    let totalScore = 0;
    ...
    for (const event of this.events) {
      ...
      // Check Mahadasha
      const mahaScore = this.scoreDashaLord(
        dashaAtEvent.mahadasha,
        significators,
        event.category
      );
      
      // Check Antardasha
      const antarScore = this.scoreDashaLord(
        dashaAtEvent.antardasha,
        significators,
        event.category
      );
  ...
  private scoreDashaLord(
    ...
    const isMaleficHouse = ['6', '8', '12'].some(h => 
      significators.secondary.some(s => s.includes(h))
    );
  ...
  private calculateTransitVerification(): { score: number; details: TransitHitDetail[] } {
    const details: TransitHitDetail[] = [];
    let totalScore = 0;
  ...
  private scoreTransitHit(
    ...
    const isSignificator = 
      significators.primary.includes(transit.planet) ||
      significators.secondary.includes(transit.planet);

    if (!isSignificator) return 0;

    // Score based on orb
    if (transit.orb <= 1) return TRANSIT_SCORES.EXACT_CONJUNCTION;
    if (transit.orb <= 3) return TRANSIT_SCORES.CLOSE_ASPECT;
    if (transit.orb <= 5) return TRANSIT_SCORES.MODERATE_ASPECT;
    if (transit.orb <= 8) return TRANSIT_SCORES.WIDE_ORB;
  ...
  private calculateDivisionalChartAlignment(): { score: number; details: VargaStrengthDetail[] } {
    const details: VargaStrengthDetail[] = [];
  ...
  private calculateVargaStrength(varga: string, keyPlanets: string[]): {
    score: number;
    ascendant: string;
    keyPlacements: string[];
  } {
    const chart = this.ephemeris.divisionalCharts?.[varga];
    if (!chart) return { score: 50, ascendant: 'Unknown', keyPlacements: [] };
  ...
  private calculateForensicMatching(): { score: number; details: ForensicAlignmentDetail[] } {
    const details: ForensicAlignmentDetail[] = [];
    let totalScore = 0;

    if (!this.forensicProfile) return { score: 50, details };
  ...
  private matchPrakriti(): number {
    ...
    if (prakriti.includes('pitta') && element === 'fire') return 90;
    if (prakriti.includes('vata') && element === 'air') return 90;
    if (prakriti.includes('kapha') && element === 'water') return 90;
    
    return 50;
  }

  private matchBuild(): number {
    ...
    if (build === 'heavy' && jupiterStrength > 100) return 85;
    if (build === 'slim' && saturnStrength > 100) return 85;
    if (build === 'athletic' && marsStrength > 100) return 85;
    
    return 60;
  }

  private matchTemperament(): number {
    const moonSign = this.ephemeris.planets.moon.sign;
    const temperament = this.forensicProfile.psychographic?.temperament;
    
    if (!temperament) return 50;

    const fierySigns = ['Aries', 'Leo', 'Sagittarius'];
    const earthySigns = ['Taurus', 'Virgo', 'Capricorn'];
    
    if (temperament === 'choleric' && fierySigns.includes(moonSign)) return 90;
    if (temperament === 'melancholic' && earthySigns.includes(moonSign)) return 90;
  ...
  private calculatePlanetaryStrength(): { score: number } {
    const shadbala = this.ephemeris.shadbala;
    if (!shadbala) return { score: 50 };
  ...
  private calculateSpecialConfigurations(): { score: number } {
    let score = 50;

    // Check for yogas (from divisional charts or separate property)
    const yogas = (this.ephemeris as any).yogas || [];
    for (const yoga of yogas) {
      if (yoga?.level === 'major') score += 10;
      else if (yoga?.level === 'minor') score += 5;
    }
  ...
  private getEventSignificators(category: string): EventSignificators {
  ...
  private getDashaAtDate(date: Date): { mahadasha: string; antardasha: string } | null {
  ...
  private getTransitsForDate(date: Date): any[] {
    // Placeholder - implement actual transit calculation
    return [];
  }

  private getPlanetaryStrength(planet: string): number {
  ...
  private getDignityInSign(planet: string, sign: string): string {
    ...
    if (this.getSignLord(sign) === planet) return 'own';
    
    return 'neutral';
  }

  private calculateHouseFromAscendant(ascendant: string, planetSign: string): number {
  ...
  private getPrakritiFromChart(): string {
    const lagnaSign = this.ephemeris.ascendant.sign;
    const fireSigns = ['Aries', 'Leo', 'Sagittarius'];
    const airSigns = ['Gemini', 'Libra', 'Aquarius'];
    const waterSigns = ['Cancer', 'Scorpio', 'Pisces'];

    if (fireSigns.includes(lagnaSign)) return 'pitta';
    if (airSigns.includes(lagnaSign)) return 'vata';
    if (waterSigns.includes(lagnaSign)) return 'kapha';
    return 'mixed';
  }

  private getBuildFromChart(): string {
    const jupiterRaw = this.ephemeris.shadbala?.['Jupiter'];
    const saturnRaw = this.ephemeris.shadbala?.['Saturn'];
    ...
    if (jupiter > 120) return 'heavy';
    if (saturn > 120) return 'slim';
    return 'medium';
  }

  private getTemperamentFromChart(): string {
    const moonSign = this.ephemeris.planets.moon.sign;
    const fireSigns = ['Aries', 'Leo', 'Sagittarius'];
    
    if (fireSigns.includes(moonSign)) return 'choleric';
    return 'balanced';
  }

  private getVargottamaPlanets(): string[] {
    const vargottama: string[] = [];
    const d1 = this.ephemeris.planets;
    const d9 = this.ephemeris.divisionalCharts?.D9?.planets;
    
    if (!d9) return vargottama;
  ...
  private getPushkarNavamsaPlanets(): string[] {
    const pushkar: string[] = [];
    const d9 = this.ephemeris.divisionalCharts?.D9?.planets;
    
    if (!d9) return pushkar;
  ...
  private determineConfidenceLevel(totalScore: number, breakdownScores: number[]): string {
    const minScore = Math.min(...breakdownScores);
    
    if (totalScore >= 90 && minScore >= 80) return 'GOD_TIER';
    if (totalScore >= 80 && minScore >= 70) return 'EXCELLENT';
    if (totalScore >= 70 && minScore >= 60) return 'GOOD';
  ...
  private collectRedFlags(
    ...
    if (dasha.score < 40) redFlags.push('Weak Dasha-Event correlation');
    if (transit.score < 40) redFlags.push('Poor transit verification');
    if (varga.score < 40) redFlags.push('Divisional chart concerns');
...
export const BTRScoring = {
  create: (
    ...
    forensicProfile: any,
    weights?: Partial<ScoringWeights>
  ) => new BTRScoringEngine(ephemeris, events, forensicProfile, weights),
