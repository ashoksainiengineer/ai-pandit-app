...
export function formatEphemerisForAI(ephemeris: EphemerisData): string {
  return `
...
function formatLifeEventsForAI(lifeEvents: LifeEvent[]): string {
  return lifeEvents
    .map(e => `- ${e.eventDate}: ${e.description} (${e.category}, importance: ${e.importance})`)
    .join('\n');
}

function createUserPrompt(ephemerisData: EphemerisData, lifeEvents: LifeEvent[], candidateTime: string): string {
...
function extractScore(analysisText: string): number {
...
export async function analyzeChartWithThinking(
  ...
  try {
    ...
    const result = await response.json() as {
      content?: Array<{ thinking?: string; text?: string }>;
    };
