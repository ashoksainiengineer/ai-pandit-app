...
export async function quickRectify(
  ...
  return {
    ...
    marginSeconds: result.marginOfErrorSeconds
  };
}
  ...
  try {
    return await TransitAnalyzer.batchAnalyze(
      ...
      })),
      {
        ...
        longitude: input.longitude,
        timezone: input.timezone,
        ascendantSign: bestCandidate.timeString
      }
    );
  } catch (e) {
...
function buildDetailedResult(
  ...
  return {
    ...
    rectifiedDate: bestCandidate.time,
    confidenceLevel: bestCandidate.confidenceLevel,
    confidencePercentage: Math.round(bestCandidate.overallScore),
    marginOfErrorSeconds: bestCandidate.marginOfErrorSeconds,
