...
export interface BoundarySafetyResult {
    ...
    nakshatraDistance: number;    // seconds from nakshatra boundary
    lagnaDistance: number;        // seconds from lagna sign change
    houseDistance: number;        // seconds from house cusp change
 ...
 */
export function verifyBoundarySafety(
    ephemeris: EphemerisData,
    julianDay: number
): BoundarySafetyResult {
    ...
    if (nakshatraDistance < SAFETY_THRESHOLDS.nakshatra) {
        ...
        recommendations.push(
            `Consider verifying with adjacent nakshatra interpretation. Current: ${currentNakshatra.name}`
        );
    }
    ...
    if (lagnaDistance < SAFETY_THRESHOLDS.lagna) {
        ...
        recommendations.push(
            `Verify if ${isNearEnd ? nextSign : ephemeris.ascendant.sign} characteristics match the native`
        );
    }
    ...
    if (!isSafe) {
        recommendations.push(
            'Consider analyzing adjacent 6-second candidates for comparison'
        );
    }
...
function getPrevSign(currentSign: string): string {
    const index = ZODIAC_SIGNS.indexOf(currentSign);
 ...
 */
export function isTimeSafeFromBoundaries(
    ephemeris: EphemerisData,
    julianDay: number
): boolean {
 ...
 */
export function formatBoundarySafetyResult(result: BoundarySafetyResult): string {
    const lines: string[] = ['BOUNDARY SAFETY ANALYSIS:'];
