    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between gap-4 flex-wrap">
                ...
                <button type="button" onClick={() => setShowModeSelector(!showModeSelector)} className="flex items-center gap-2 px-4 py-2 bg-[#F5EFE7] hover:bg-[#D4AF37]/10 rounded-xl text-sm transition-colors border border-[#F0E8DE] hover:border-[#D4AF37]/50">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 p-4 bg-[#F5EFE7] rounded-xl border border-[#F0E8DE] shadow-lg animate-fade-in">
                    {PRECISION_OPTIONS.map((opt) => (
                        <button key={opt.value} type="button" onClick={() => handleModeChange(opt.value)}
                            className={`p-3 rounded-xl text-left transition-all ${mode === opt.value ? 'bg-[#D4AF37]/20 border-2 border-[#D4AF37]' : 'bg-white border-2 border-transparent hover:border-[#D4AF37]/30'}`}>
            ...
            <div className="bg-[#F5EFE7] p-4 rounded-xl border border-[#F0E8DE]">
                ...
                {showEncouragement && <div className="mt-3 flex items-center gap-2">{isComplete() ? <><span className="w-3 h-3 rounded-full bg-[#2D7A5C] animate-pulse" /><span className="text-xs text-[#2D7A5C] font-medium">✓ Date recorded</span></> : <><span className="w-3 h-3 rounded-full bg-[#F59E0B]" /><span className="text-xs text-[#F59E0B] font-medium">Please complete the date</span></>}</div>}
            </div>
