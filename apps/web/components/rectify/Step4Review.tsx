      ...
      <div className="bg-white rounded-xl p-6 border border-[#E8E0D5]">
        ...
        {events.length === 0 ? (
          ...
          <ul className="space-y-2">
            {events.map((event) => (
              <li key={event.id} className="text-sm text-[#636363]">
      ...
      <div className="bg-white rounded-xl p-6 border border-[#E8E0D5]">
        ...
        {traits ? (
          <div className="grid grid-cols-2 gap-3 text-sm text-[#3A3530]">
            ...
            <div><strong className="text-[#636363] text-xs">Jawline:</strong> <span className="capitalize">{traitLabel('jawLine' in fs ? (fs as Record<string,string>).jawLine : undefined)}</span></div>
            <div><strong className="text-[#636363] text-xs">Lips:</strong> <span className="capitalize">{traitLabel('lips' in fs ? (fs as Record<string,string>).lips : undefined)}</span></div>
            <div><strong className="text-[#636363] text-xs">Ears:</strong> <span className="capitalize">{traitLabel('ears' in fs ? (fs as Record<string,string>).ears : undefined)}</span></div>
            <div><strong className="text-[#636363] text-xs">Voice:</strong> <span className="capitalize">{traitLabel('voicePitch' in fs ? (fs as Record<string,string>).voicePitch : undefined)}</span></div>
            <div><strong className="text-[#636363] text-xs">Skin:</strong> <span className="capitalize">{traitLabel(sh.complexion)}</span></div>
            <div><strong className="text-[#636363] text-xs">Hair:</strong> <span className="capitalize">{traitLabel(sh.hairType)}</span></div>
            <div><strong className="text-[#636363] text-xs">Build:</strong> <span className="capitalize">{traitLabel(traits.build)}</span></div>
            <div><strong className="text-[#636363] text-xs">Height:</strong> {ht && typeof ht === 'object' && (ht as {cm:number}).cm ? `${(ht as {cm:number}).cm} cm` : '—'}</div>
            {sh.marks && sh.marks.length > 0 && (
              <div className="col-span-2"><strong className="text-[#636363] text-xs">Special Marks:</strong> {sh.marks.join(', ')}</div>
            )}
          </div>
        ) : (
          <p className="text-sm text-[#636363]">No physical traits recorded.</p>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl p-6 border border-[#E8E0D5]">
        ...
        {forensicTraits ? (
          <div className="grid grid-cols-2 gap-3 text-sm text-[#3A3530]">
            ...
            <div><strong className="text-[#636363] text-xs">Decision Making:</strong> <span className="capitalize">{forensicTraits.psychographic?.decisionMaking || '—'}</span></div>
            <div><strong className="text-[#636363] text-xs">Stress Response:</strong> <span className="capitalize">{forensicTraits.psychographic?.stressResponse || '—'}</span></div>
            <div><strong className="text-[#636363] text-xs">Sibling Position:</strong> <span className="capitalize">{forensicTraits.family?.siblingPosition || '—'}</span></div>
            <div><strong className="text-[#636363] text-xs">Brothers:</strong> {forensicTraits.family?.brotherCount ?? '—'}</div>
            <div><strong className="text-[#636363] text-xs">Sisters:</strong> {forensicTraits.family?.sisterCount ?? '—'}</div>
          </div>
        ) : (
          <p className="text-sm text-[#636363]">No forensic traits recorded.</p>
        )}
      </div>
