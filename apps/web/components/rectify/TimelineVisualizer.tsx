    ...
    return (
        <div className="relative">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-[#78611D] flex items-center gap-2">
                ...
                {sortedEvents.length >= 8 && (
                    <span className="px-4 py-2 rounded-full bg-[#184131]/20 text-[#184131] text-sm font-bold border border-[#184131]/30">✨ Rich Timeline!</span>
                )}
            </div>

            <div className="relative pl-10">
                <div className="absolute left-4 top-0 bottom-0 w-1 bg-gradient-to-b from-[#78611D] via-[#8B5CF6] to-transparent rounded-full" />
                    ...
                    {sortedEvents.map((event, index) => {
                        ...
                        return (
                            <div key={event.id} onClick={() => handleEventClick(event.id)} className={`relative group cursor-pointer transition-all duration-300 ${isSelected ? 'scale-[1.02]' : 'hover:scale-[1.01]'}`}>
                                <div className={`absolute -left-6 top-4 w-5 h-5 rounded-full ${getImportanceColor(event.importance)} shadow-lg transition-transform ${isSelected ? 'scale-125 ring-4 ring-[#78611D]/30' : ''}`} />
                                <div className={`ml-4 p-5 rounded-xl border-2 transition-all ${isSelected ? 'bg-[#78611D]/10 border-[#78611D]/50 shadow-lg' : 'bg-[#F5EFE7] border-transparent hover:border-[#78611D]/30'}`}>
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between gap-2 flex-wrap">
                                                ...
                                                <span className="text-xs px-3 py-1 rounded-full bg-white text-[#78611D] font-mono font-medium">{formatDisplayDate(event)}</span>
                                            </div>
                                            {event.description && <p className="text-sm text-[#7A756F] mt-2 line-clamp-2">{event.description}</p>}
                                            <div className="flex items-center gap-2 mt-3">
                                                <span className={`text-xs px-2 py-1 rounded-full font-medium ${event.importance === 'critical' ? 'bg-[#184131]/20 text-[#184131]' : event.importance === 'high' ? 'bg-[#8B5CF6]/20 text-[#8B5CF6]' : 'bg-[#78611D]/20 text-[#78611D]'}`}>{event.importance?.toUpperCase()}</span>
                                                <span className="text-xs text-[#7A756F]">{event.category?.replace(/_/g, ' ')}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                {isFirst && <div className="absolute -left-20 top-4 text-xs text-[#78611D] font-bold bg-[#78611D]/10 px-2 py-1 rounded">🏁 FIRST</div>}
                                {isLast && sortedEvents.length > 1 && <div className="absolute -left-20 top-4 text-xs text-[#184131] font-bold bg-[#184131]/10 px-2 py-1 rounded">🎯 LATEST</div>}
                            </div>
                        );
                    })}
                </div>
            ...
            {sortedEvents.length >= 3 && (
                <div className="mt-8 p-5 bg-[#F5EFE7] rounded-xl flex items-center justify-around text-center border border-[#F0E8DE]">
                    <div><div className="text-3xl font-bold text-[#78611D]">{sortedEvents.length}</div><div className="text-xs text-[#7A756F]">📅 Events</div></div>
                    <div className="w-px h-10 bg-[#F0E8DE]" />
                    <div><div className="text-3xl font-bold text-[#184131]">{sortedEvents.filter(e => e.importance === 'critical').length}</div><div className="text-xs text-[#7A756F]">⭐ Critical</div></div>
                    <div className="w-px h-10 bg-[#F0E8DE]" />
            ...
            {sortedEvents.length >= 10 && <div className="mt-6 p-4 bg-[#184131]/10 border border-[#184131]/30 rounded-xl flex items-center gap-3"><span className="text-3xl">🏆</span><p className="text-sm text-[#184131] font-medium">Excellent! Your timeline is comprehensive for maximum accuracy!</p></div>}
        </div>
    );
});
