  ...
  loading: () => (
    <div className="w-full h-48 bg-[#F5EFE7] rounded-lg flex items-center justify-center border border-[#E8E0D5]">
      <div className="text-center">
        ...
        <p className="text-[#5A554F] text-xs mt-2">Loading map...</p>
      </div>
    </div>
  )
});
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-1 p-1 bg-[#F5EFE7] rounded-lg border border-[#E8E0D5]">
        {[
          ...
          const Icon = m.icon;
          return (
            <button key={m.id} type="button" onClick={() => setMode(m.id as InputMode)} className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-md text-xs font-medium transition-all ${mode === m.id ? 'bg-white text-[#B8860B] shadow-sm border border-[#E8E0D5]' : 'text-[#5A554F] hover:text-[#4A453F]'}`}>
        ...
        <div className="relative" ref={dropdownRef}>
          <div className="relative">
            <input
              ...
              className="w-full h-10 px-4 pr-20 bg-white border border-[#E8E0D5] rounded-lg text-[#1A1612] text-sm placeholder-[#8A857F] focus:border-[#78611D] focus:ring-2 focus:ring-[#78611D]/10 outline-none"
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
              {searchQuery && (
                ...
                >
                  <X className="w-4 h-4 text-[#8A857F]" />
                </button>
              )}
              {isSearching && (
          ...
          {showDropdown && searchResults.length > 0 && (
            <div className="absolute z-50 w-full mt-2 bg-white border border-[#E8E0D5] rounded-lg shadow-lg max-h-64 overflow-y-auto">
              {searchResults.map((result) => (
                ...
                >
                  <div className="font-medium text-[#1A1612] text-sm">{result.city || 'Unknown Location'}{result.district && <span className="text-[#5A554F]"> • {result.district}</span>}</div>
                  <div className="text-xs text-[#5A554F]">{[result.state, result.country].filter(Boolean).join(', ')}</div>
          ...
          {selectedLocation && (
            <div className="mt-3 p-3 bg-[#184131]/5 border border-[#184131]/20 rounded-lg">
              <div className="flex items-center gap-2 text-[#184131] text-xs font-semibold mb-2"><span>✓</span><span>Location Selected</span></div>
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                <div><span className="text-[#5A554F]">City:</span><span className="text-[#1A1612] ml-1 font-medium">{selectedLocation.city}</span></div>
                {selectedLocation.district && (<div><span className="text-[#5A554F]">District:</span><span className="text-[#1A1612] ml-1">{selectedLocation.district}</span></div>)}
                <div><span className="text-[#5A554F]">State:</span><span className="text-[#1A1612] ml-1">{selectedLocation.state}</span></div>
                <div><span className="text-[#5A554F]">Country:</span><span className="text-[#1A1612] ml-1">{selectedLocation.country}</span></div>
              </div>
            </div>
          )}
        </div>
      ...
      {mode === 'manual' && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] text-[#5A554F] mb-1 uppercase tracking-wider">Latitude</label>
              <input type="number" step="0.0001" min="-90" max="90" value={manualLat} onChange={(e) => setManualLat(e.target.value)} className="w-full h-10 px-3 bg-white border border-[#E8E0D5] rounded-lg text-[#1A1612] text-sm font-mono focus:border-[#78611D] outline-none" placeholder="28.6139" />
            </div>
            <div>
              <label className="block text-[10px] text-[#5A554F] mb-1 uppercase tracking-wider">Longitude</label>
              <input type="number" step="0.0001" min="-180" max="180" value={manualLng} onChange={(e) => setManualLng(e.target.value)} className="w-full h-10 px-3 bg-white border border-[#E8E0D5] rounded-lg text-[#1A1612] text-sm font-mono focus:border-[#78611D] outline-none" placeholder="77.2090" />
            </div>
          </div>
          <div>
            <label className="block text-[10px] text-[#5A554F] mb-1 uppercase tracking-wider">Timezone</label>
            <select value={manualTimezone} onChange={(e) => setManualTimezone(e.target.value)} className="w-full h-10 px-3 bg-white border border-[#E8E0D5] rounded-lg text-[#1A1612] text-sm focus:border-[#78611D] outline-none">
      ...
      {mode === 'map' && (
        <div className="space-y-3">
          <div className="flex gap-2">
            <input type="text" value={mapQuery} onChange={(e) => setMapQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleMapSearch()} className="flex-1 h-10 px-4 bg-white border border-[#E8E0D5] rounded-lg text-[#1A1612] text-sm focus:border-[#78611D] outline-none" placeholder="Search location..." />
            <button type="button" onClick={handleMapSearch} className="px-4 h-10 bg-gradient-to-r from-[#B8860B] to-[#78611D] text-white font-semibold rounded-lg hover:shadow-md transition-all text-sm">Search</button>
          </div>
          ...
          {mapMarker && (
            <div className="p-3 bg-[#184131]/5 border border-[#184131]/20 rounded-lg">
              <div className="flex items-center gap-2 text-[#184131] text-xs font-semibold mb-1"><MapPin className="w-3.5 h-3.5" /><span>Selected</span></div>
              <div className="font-mono text-xs text-[#1A1612]">Lat: {mapMarker.lat.toFixed(6)}°, Lng: {mapMarker.lng.toFixed(6)}°</div>
            </div>
          )}
          <p className="text-[10px] text-[#5A554F] flex items-center gap-1"><span>💡</span> Search city, then click map to fine-tune</p>
        </div>
      )}
      ...
      {(latitude && longitude && mode !== 'search') && (
        <div className="p-3 bg-[#F5EFE7] rounded-lg border border-[#E8E0D5]">
          <div className="text-[10px] text-[#5A554F] mb-1 uppercase tracking-wider">Selected Location</div>
