...
export default function AdminDashboardError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error('Admin dashboard error:', error); }, [error]);
  return (
    <main className="min-h-screen flex items-center justify-center p-8">
      <div className="text-center">
        <h1 className="text-2xl font-medium text-black mb-2">Dashboard unavailable</h1>
        ...
        <div className="flex gap-4 justify-center">
          ...
          <Link href="/admin/dashboard" className="px-6 py-2 border rounded-xl">Reload</Link>
        </div>
      </div>
    </main>
  );
