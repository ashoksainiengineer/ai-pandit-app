...
export async function GET(_req: NextRequest) {
    ...
    try {
        ...
        const parsedSessions = userSessions.map(s => ({
            ...
            fullName: parseSensitiveField(s.fullName, undefined, user.id),
            lifeEvents: parseSensitiveField(s.lifeEvents, [], user.id),
            forensicTraits: parseSensitiveField(s.forensicTraits, undefined, user.id),
            spouseData: parseSensitiveField(s.spouseData, undefined, user.id),
            offsetConfig: parseSensitiveField(s.offsetConfig, undefined, user.id),
            // Reconstruct birthData object
            birthData: {
                fullName: parseSensitiveField(s.fullName, undefined, user.id),
                dateOfBirth: parseSensitiveField(s.dateOfBirth, undefined, user.id),
                tentativeTime: parseSensitiveField(s.tentativeTime, undefined, user.id),
                birthPlace: parseSensitiveField(s.birthPlace, undefined, user.id),
                latitude: s.latitude,
