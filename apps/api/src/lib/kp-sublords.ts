...
// CORE CALCULATIONS
 ...
 export function calculateKPSubLords(longitude: number): KPSubLordData {
  ...
  // Level 2: Sub-Lord
  const subLordResult = calculateSubDivision(
    positionInNakshatra,
    starLord,
    NAKSHATRA_SPAN
  );
  ...
  const subSubLordResult = calculateSubDivision(
    subLordResult.positionInDivision,
    subLordResult.lord,
    subLordResult.span
  );
  
  // Level 4: Sub-Sub-Sub-Lord (seconds-level)
  const subSubSubLordResult = calculateSubDivision(
    subSubLordResult.positionInDivision,
    subSubLordResult.lord,
    subSubLordResult.span
  );
 ...
 */
export function calculateKPCuspalSubLords(
 ...
 */
export function correlateEventWithKP(
  event: { id: string; date: Date; category: string },
  dashaLord: string,
  cuspalSubLords: KPCuspalData[],
  significatorLongitudes: Record<string, number>
): KPEventCorrelation {
  ...
  let score = 0;
  let precision: 'exact' | 'close' | 'approximate' = 'approximate';
...
function getTargetHouseForEvent(category: string): number {
...
function createEmptyCorrelation(event: { id: string; date: Date }): KPEventCorrelation {
  return {
    eventId: event.id,
    eventDate: event.date,
    dashaLord: 'Unknown',
 ...
 */
export function calculateKPSubLordsBatch(
  longitudes: number[]
): Map<number, KPSubLordData> {
  const results = new Map<number, KPSubLordData>();
  const cache = new Map<string, KPSubLordData>();
 ...
 */
export function findKPSubLordWindow(
  baseTime: Date,
  targetSubLord: string,
  searchWindowHours: number,
  ephemerisCalculator: (time: Date) => number  // Returns Moon longitude or other significator
): { start: Date; end: Date; confidence: number } | null {
  const window: { start: Date; end: Date; confidence: number } = {
  ...
  const checks: { time: Date; longitude: number; subLord: string }[] = [];
  ...
  const matchingPeriods: { start: Date; end: Date }[] = [];
  ...
  const bestPeriod = matchingPeriods.reduce((a, b) => 
